...
class Model(object):
    ...
    def _build_model(self):
        ...
        # (3) jinuk
        with tf.variable_scope("gru"):
            gru_outputs = self.gru(self.input_x, scope="gru")  # [b, t, d]    
        last_hidden = gru_outputs[:,-1,:]
        last_hidden = tf.layers.dropout(last_hidden, self.config.dropout)

        linear_input = tf.reshape(self.input_x, shape=[-1, self.config.nsteps * self.config.nfeatures])
        projection = tf.layers.dense(linear_input, self.config.hidden_size, tf.nn.relu, kernel_initializer=layers.xavier_initializer(), use_bias = True)
        projection = tf.layers.dropout(projection, self.config.dropout)

        last_input = self.input_x[:,-1,:]
        self.impact_gate = tf.layers.dense(last_input, 1, tf.nn.sigmoid, kernel_initializer=layers.xavier_initializer(), use_bias = False)
                
        # self.input_ = tf.layers.dropout(self.input_, self.config.dropout)
        linear_input = self.impact_gate*last_hidden + (1-self.impact_gate)*projection

        # linear_input = tf.concat([last_hidden, projection], axis=1)
        result = tf.squeeze(tf.layers.dense(linear_input, 1, tf.nn.tanh, use_bias=False))
        self.predictions = result
        
        # self.predictions = (1.0 -self.config.ar_g) * result + self.config.ar_g * self.dense_result
