...
class Model(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("inputs"):
            ...
            input_ar = tf.layers.dense(self.input, 32, None, use_bias = True)
        ...
        with tf.variable_scope("memories"):
            ...
            # memory_ar : [b*m, nf]
            memory_ar, ar_loss_ = self.auto_regressive(memories, self.config.ar_lambda, self.config.x_len, scope="memories")
            
        # context : [b, nf]
        context = self.attention(input_ar, memory_ar)
        linear_inputs = tf.concat([input_ar, context], axis=1) # [b, 2nf]
        linear_inputs = input_ar
        #self.predictions = tf.layers.dense(self.input, 1, activation=tf.nn.tanh, kernel_initializer=layers.xavier_initializer(), use_bias=False)
        self.predictions = tf.layers.dense(self.input, 1, activation=None,  use_bias=False)
    ...
    def add_placeholder(self):
        ...
        self.memories = tf.placeholder(shape=[None, (self.config.nsteps) * self.config.msteps, self.config.nfeatures], dtype=tf.float32,
                                      name="memories")
    ...
    def attention(self, inputs, memories):
        ...
        # [b, 1, attention_size]
        query = tf.layers.dense(query, self.config.attention_size, activation=None, use_bias=False,
        ...
        # [b, m, attention_size]
        key = tf.layers.dense(key, self.config.attention_size, activation=None, use_bias=False,
                                kernel_initializer=layers.xavier_initializer(),
                                kernel_regularizer=self.regularizer)
        bias = tf.get_variable(shape=[self.config.attention_size], initializer=tf.zeros_initializer(), name="attention_bias")
